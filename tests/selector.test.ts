...
describe('stripMarkdown', () => {
	...
	test('removes embeds entirely', () => {
		...
		assert.equal(stripMarkdown('![alt](https://example.com/a.png)\nCaption.'), 'Caption.');
	});

	test('drops a fenced code block rather than showing it as a wall of syntax', () => {
		const note = ['Before.', '```ts', 'const x = 1;', '```', 'After.'].join('\n');
		assert.equal(stripMarkdown(note), 'Before.\n\nAfter.');
	});

	test('drops the markers of quotes, bullets and inline code', () => {
	...
	test('collapses the blank runs that dropping things leaves behind', () => {
		assert.equal(stripMarkdown('One.\n\n\n\nTwo.'), 'One.\n\nTwo.');
	});

	/**
	 * What is searched has to be what the reader could have seen. A hit inside a URL
	 * or an image filename would highlight nothing on the card.
	 */
	test('the haystack holds only what a plain cover shows', () => {
		assert.equal(stripMarkdown('see [text](https://example.com/secret)').includes('secret'), false);
		assert.equal(stripMarkdown('![[Pasted image 20260810.png]]').includes('20260810'), false);
	});
});
